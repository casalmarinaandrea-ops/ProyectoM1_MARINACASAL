...
function renderPalette() {
  ...
  currentPalette.forEach((color, index) => {
    ...
    colorCard.setAttribute(
      "aria-label",
      `Color ${color.hex}, ${color.hsl}. Click para copiar`
    );
    ...
    lockButton.addEventListener("click", (event) => {
      ...
      renderPalette();
      showToast(
        currentPalette[index].locked ? "Color bloqueado" : "Color desbloqueado"
      );
    });
...
function copyColor(color) {
  ...
  if (copyFormat === "hex") {
    ...
    textToCopy = `${color.hex} | ${color.hsl}`;
  }

  navigator.clipboard.writeText(textToCopy);
  showToast(`${textToCopy} copiado`);
}
